...
- 自带依赖（Portable version）：`artifacts/FileLocksmith Portable version/<平台>/<配置>/`
- 系统依赖（System dependency versions）：`artifacts/FileLocksmith System dependency versions/<平台>/<配置>/`

## 打包流程

1. 构建 UI，确保 `x64/Release/WinUI3Apps`（Portable）与 `x64/Release/WinUI3Apps.System`（System）有产物
...
- Windows App SDK Runtime 与 VC++ 运行库（仅 Portable version，脚本会尝试复制）

## 注意事项

- 便携包必须包含 `WinUI3Apps` 下的全部资源目录（含语言包），否则可能在打开页面时崩溃。
- System dependency versions 不包含 VC++ 运行库（依赖系统已安装）。
